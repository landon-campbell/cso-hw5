...
int main() {
    ...
    // set the array to access three addresses that map to the same set
    global_array[0] = 4096; // 0 -> 4096
    global_array[4096] = 8192; // 4096 -> 8192
    global_array[8192] = 0; // 8192 -> 0

    // prevent the compiler from optimizing based on knowing the array values
    prevent_optimizations_based_on_knowing_array_values();

    // access the array in a loop
    int j = 0; // start at index 0

    for (int i = 0; i < ten_mil; ++i) { // loop ten million times
        j = global_array[j]; // access the next element
    }
